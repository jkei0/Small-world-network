...
def model_orig(obs_space, num_outputs, inp):

    #first layer
    mat1 = np.ones((inp, NUM_NODES))
    
    #second layer
    mat2 = np.zeros((NUM_NODES+inp, NUM_NODES))
    mat2[inp:,:] = 1
    
    #third layer
    mat3 = np.zeros((NUM_NODES*2+inp, NUM_NODES))
    mat3[NUM_NODES+inp:,:] = 1
        
    #output layer
    mat4 = np.zeros((NUM_NODES*3+inp, num_outputs))
    mat4[NUM_NODES*2+inp:,:] = 1
...
def model_orig_value(obs_space, inp):
    ...
    #output layer
    mat4 = np.zeros((NUM_NODES*3+inp, 1))
    mat4[NUM_NODES*2+inp:,:] = 1
...
class SmallWorldModel(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs,
        ...
        swmat_value = rewire_num_connections(value_mat, value_layers, 0)
...
#    def rewired_model(self, layers, obs_space, num_outputs, value_layers):
#        
#        #input layer
#        input_layer = Input(shape=obs_space.shape)
#        
#        #first layer
#        layer_1 = CustomConnected(NUM_NODES, connections=layers[2], activation='tanh')(input_layer)
#        value_1 = CustomConnected(NUM_NODES, connections=value_layers[2], activation='tanh')(input_layer)
#        z_layer = concatenate([input_layer, layer_1])
#        z_value = concatenate([input_layer, value_1])
#        
#        #second layer
#        layer_2 = CustomConnected(NUM_NODES, connections=layers[1], activation='tanh')(z_layer)
#        value_2 = CustomConnected(NUM_NODES, connections=value_layers[1], activation='tanh')(z_value)     
#        z_layer = concatenate([z_layer, layer_2])
#        z_value = concatenate([z_value, value_2])
#        
#        #output layer
#        layer_out = CustomConnected(num_outputs, connections=layers[0], activation=None)(z_layer)
#        value_out = CustomConnected(1, connections=value_layers[0], activation=None)(z_value)
#        
#        self.base_model = Model(input_layer, [layer_out, value_out])
#        
#        self.register_variables(self.base_model.variables)
...
if __name__== "__main__":
    ...
    ray.init(memory=1024*1024*5000)
