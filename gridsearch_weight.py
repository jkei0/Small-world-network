"""
Created on Wed May  6 23:54:50 2020
...
PATH = 'scratch/project_2002707/gridsearch_dropout/Sensorless_drive_diagnosis.txt'
...
class DropoutModel:
    ...
    def __init__(self, layers, la1, la2):
        self.layers = layers
        self.la1 = la1
        self.la2 = la2
    ...
    def create_model(self):
        ...
        #first layer
        l1 = Dense(NUM_NODES, activation='relu', kernel_regularizer=regularizers.l2(self.la2), 
                        activity_regularizer=regularizers.l1(self.la1))(inp)
        
        #second
        l2 = CustomConnected(NUM_NODES, connections=self.layers[5], activation='relu', kernel_regularizer=regularizers.l2(self.la2), 
                        activity_regularizer=regularizers.l1(self.la1))(l1)
        z = concatenate([l1, l2])
        
        #third
        l3 = CustomConnected(NUM_NODES, connections=self.layers[4], activation='relu', kernel_regularizer=regularizers.l2(self.la2), 
                        activity_regularizer=regularizers.l1(self.la1))(z)
        z = concatenate([z,l3])
        
        #forth 
        l4 = CustomConnected(NUM_NODES,connections=self.layers[3], activation='relu', kernel_regularizer=regularizers.l2(self.la2), 
                        activity_regularizer=regularizers.l1(self.la1))(z)
        z = concatenate([z, l4])
        
        #fifth
        l5 = CustomConnected(NUM_NODES, connections=self.layers[2], activation='relu', kernel_regularizer=regularizers.l2(self.la2), 
                        activity_regularizer=regularizers.l1(self.la1))(z)
        z = concatenate([z, l5])
        
        #sixth
        l6 = CustomConnected(NUM_NODES, connections=self.layers[1], activation='relu', kernel_regularizer=regularizers.l2(self.la2), 
                        activity_regularizer=regularizers.l1(self.la1))(z)
        z = concatenate([z, l6])
        
        
        #output
        out = CustomConnected(OUTPUT, connections=self.layers[0], activation='softmax')(z)
...
def create_model(layers, dropout=0.1, dropout_in=0.1):
...
if __name__ == '__main__':
    ...
    la1 = [0.0000000001, 0.000000001, 0.00000001, 0.0000001, 0.000001, 0.00001, 0.0001, 0.001]
    la2 = [0.0000000001, 0.000000001, 0.00000001, 0.0000001, 0.000001, 0.00001, 0.0001, 0.001]
    ...
    for d in la1:
        for d_i in la2:
            model = DropoutModel(new_layers, d, d_i)
            history, results = model.test_model()
            param_grid[d, d_i] = results[1]
             
    
    for i in param_grid:
        print("{}, {}".format(i,param_grid[i]))
#
